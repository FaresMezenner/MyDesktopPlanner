...
public class Creneau implements Decomposable, Serializable {
    ...
    public void decomposer(){
        ...
        } else if (nouvelle_fin.compareTo(getFin()) == 0) {
            System.out.println("On ne décompose pas le creneau , car la tache le recouvre entiérement");
        } else {
        ...
        if (this.libre){
            if (tache.isDecomposable()){
                ...
                if (tache.getDuree().compareTo(Duration.between(getDebut(),getFin())) > 0){
                    ...
                    TacheDecomposable nouvelle_tache = new TacheDecomposable(tacheDec.getNom(),tacheDec.getDuree().minus(Duration.between(getDebut(),getFin())),tacheDec.getPriorite(),tacheDec.getDateLimite(),tacheDec.getCategorie(),tacheDec.getEtat(),tacheDec.getNumeroSousTache());
                        // On insere la tache dans le creneau
                    tacheDec.decomposer();
                    tacheDec.setSuivant(nouvelle_tache);
                    nouvelle_tache.setPrecedent(tacheDec);
                    this.tache = tacheDec;
